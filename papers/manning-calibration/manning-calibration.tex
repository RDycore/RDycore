...
% [CLAUDE: CaMa-Flod-GPU is not differentiable]
% [CLAUDE-REPLY/DX: correct, and thank you -- checked the GMD paper, it
%  reports no adjoint. Moved out of the differentiable list into its own
%  clause as a GPU port. Kept the citation because it makes our point
%  better there: the GPU work exists and *wants* the gradient. Please
%  confirm the "future work" characterization is fair to Kang et al.]
...
rather than background to it, and Table~\ref{tab:configs} collects the
...
production-window spectrum. Ten classes learn $5\%$ or less: any value
a calibration reports for them is the prior speaking, not the data.}
\label{tab:learned}
\end{table}
...
% [Emil, 2026-09-06: Use tab:scaling as a benchmark with independent,
%  equal-variance residual errors and comparable per-mark sensitivities
%  for the fixed prior and linearization; do not frame it as a prediction
%  for the actual expanded datasets. Correlated residual errors enter
%  through Gamma^{1/2} S^T C^{-1} S Gamma^{1/2}. No separate OED study
%  needed if claims stay restricted to the benchmark; the count of
%  data-informed combinations is the important part. Scaling arithmetic:
%  71 and 108 marks give 2 and 3 directions with lambda>1; 46 marks at
%  sigma=0.10 m also gives 3.]
% [CLAUDE-REPLY/Emil: your three counts are right and ours were stale --
%  tab:scaling and tab:learned were computed from the two-hour PILOT
%  spectrum (lambda_3 = 0.33) and never recomputed when the production
%  spectrum (lambda_3 = 0.61) replaced it on 08-29; the 46-mark count was
%  the only row rechecked. Both tables now use the production spectrum
%  (reproduced from the o61 dumps: 2.779, 0.682, 0.613). Benchmark
%  framing and the covariance form added to the caption; no OED study.
%  One caveat we will add once the prior question settles: the count
%  depends on the prior width (1 at +/-15%, 2 at +/-20%, 3 at +/-30%,
%  uniform relative), so we will report it alongside the width.]
...
\begin{enumerate}
  \item \textbf{How far can the NLCD Manning lookup be wrong, in
        percent --- and for which classes?} \emph{This one sets a number
        in the method directly.} The prior width is the regularization:
        $\beta = \sigma^{-2}$. Two facts to answer against. First, what
        has actually been run: every calibration and the spectrum in
        this draft use an \emph{absolute} width $\sigma_n = 0.015$, which
        is $\pm 12.5\%$ on developed-medium --- the class that carries
        the leading eigenvector --- but $\pm 9\%$ on developed-high and
        $\pm 56\%$ on barren. Under that prior the first quasi-Newton
        step drove developed-low up $2.6\times$ and developed-medium to
        its floor ($n = 0.036$, smoother than concrete), an excursion
        ordered by prior value rather than by hydrology
        (Section~\ref{sec:calibration}). The uniform relative width the
        text describes is what we intend; a calibration under it costs
        one 2-node, 6-hour Perlmutter slot (12 GPU node-hours) per width
        per parameterization, and we will run the agreed width once
        this question is answered rather than guess at it.
        Second, what the spectrum already says, since its prior enters
        only in the whitening: under a uniform relative width these 46
        marks support one roughness combination at $\pm 15\%$, two at
        $\pm 20\%$, three at $\pm 30\%$, four at $\pm 50\%$. \emph{If
        $\pm 15\%$ is defensible}, the one-parameter reading and the
        calibration stand as reported. \emph{If the honest width is
        $\pm 30\%$} --- the answer below --- the marks determine three
        combinations, which is the three-class active set of
        Section~\ref{sec:spectrum}, and the headline becomes that
        calibration; what a $\pm 30\%$ prior cannot support is the
        \emph{fifteen}-class field, whose twelve undetermined directions
        it releases. The part of the answer we still need is whether
        $\pm 30\%$ is meant for the developed classes specifically,
        which dominate both the area and the spectrum.
\textcolor{red}{Donghui: I think $\pm 30\%$ or more is possible.}
  \item \textbf{Is $15\%$ of peak-WSE error being roughness-attributable
        ...
        looked at.
\textcolor{red}{Donghui: I think precipitation and DEM uncertainties should dominate the WSE error.Especially in the river channels, any bias of bathymetry will directly convert to WSE error.}
...
% [Donghui, 2026-09-06: check the elevation of those 37 cells and their
%  neighbours -- if they are local depressions, water could legitimately
%  accumulate for 72 h. Also check whether the rainfall series lasted
%  more than 72 h.]
% [CLAUDE-REPLY/Donghui: both checked on the 30 m mesh and the 72-h run;
%  the measurements are now in Section 6.1. Not depressions: 1 of the 37
%  cells is lower than all its edge-neighbours, versus 6 of the 71 marks
%  that crest normally. Not rain: simulation hours 60-72 carry 1.7% of
%  the event and hours after 72 carry none (basin-mean MRMS, hour 0 =
%  2017-08-26 18:00). What they are is a lake: the model's peak water
%  surface across all 37 is 20.0 +/- 1.1 m over surveyed peaks of
%  6.2-20.2 m, and 26% of the perimeter lies below 20 m while the exit
%  is 13 element edges wide. The reach ponds to the height its walls
%  impose.]
...
\textcolor{red}{Donghui: 20\% error in antecedent water storage is possible. Actually, the initial condition remains highly uncertainty but can significantly affect the prediction accuracy in the next few days. It will be an interesting study to infer the IC based on target.} 
  \item \textbf{Is the calibrated field itself acceptable?} We can
        ...
        and woody wetland ($30\%$ and $28\%$ uncertainty reductions),
        three others weakly, and ten classes by $5\%$ or less, so those
        ten should be frozen on physical grounds rather than left for
        an optimizer to move.
\end{enumerate}
...
\textcolor{red}{Donghui: There is a mismatch between of the spatial resoltion of the high water marks (point) and mesh. So, I would suggest to calibrate with gauge observation.} 
...
\textcolor{red}{Donghui: I prefer calibrating the Manning coefficient per NLCD class instead of per cell.}
