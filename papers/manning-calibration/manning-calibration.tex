...
error, and only at roughness values no hydrologist would defend;
fifteen land-cover classes calibrated separately remove $0.107$\,m,
$15\%$ of the error. The spectrum of the prior-preconditioned
Gauss--Newton Hessian, from sixteen forward runs, shows why: at the
$\pm 30\%$ width the land-cover lookup deserves, the marks determine
about three combinations of the fifteen classes, and calibrating those
three recovers $85\%$ of what fifteen achieve, with objective and error
metrics in agreement where the fifteen-class field's are not.
% [PENDING o62: refresh 85% / 0.6274 with the sigma_alpha = 0.30 numbers]
...
holds the bank of an incised bayou, not its channel.
% [PENDING o63: one sentence -- does the gauge-calibrated field predict the marks?]
...
\begin{itemize}
  ...
  % [PENDING o63: a validation item -- gauge-calibrated, mark-validated]
  ...
  \item a catalog of the nondifferentiable structures a production
        flood code presents to an adjoint (Section~\ref{sec:nondiff}).
\end{itemize}
...
drives the misfit down by a factor of $2\times10^{7}$ --- the
synthetic observations carry no noise, and are fit to roundoff ---
while the recovered field is
...
% [PENDING o63: the gauge-calibrated / mark-validated design and its result]
...
discrepancy, and about three at the width the lookup deserves
