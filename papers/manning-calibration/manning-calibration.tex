...
roughness field.} We report the prior width used with every number. Its
value is a judgment about the land-cover lookup, not a tuning
parameter, and the domain team's judgment is that the lookup can be
wrong by $30\%$ or more; Section~\ref{sec:spectrum} turns that width
into a count of the roughness combinations the marks determine.
...
something the optimizer can settle. The domain team puts the lookup's
uncertainty at $30\%$ or more, which admits the request for the classes
the marks constrain and, as the spectrum shows, releases twelve that
they do not.
...
the forcing data, not for the optimizer. The domain team's answer is
that it is: the antecedent state after a 29-hour spin-up under radar
rainfall is among the least certain inputs the model has, and an error
of that size in it is plausible.
