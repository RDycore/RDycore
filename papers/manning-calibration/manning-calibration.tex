...
that is still only $15\%$ of the error. Sixteen forward runs --- one
per land-cover class, perturbing that class and watching how the 46
modelled peaks respond, plus an unperturbed baseline --- show why it
cannot be more: the marks carry about \emph{one} parameter's worth
...
We suggest making this a routine step in any calibration study:
before running an optimizer, measure --- in a handful of forward runs
--- how much of the model's error the parameter is able to remove at
best, and report that ceiling beside the reduction actually achieved.
It is common to report only the reduction; without the ceiling, a
successful-looking calibration can hide that most of the model's
error was never the parameter's to remove.
\end{abstract}
...
values on which the data out-weighs the prior, and no second. The
scans and the spectrum agree, and they are
...
Two details sharpen it. The remaining $16\%$ is real, not noise, and
it decomposes exactly: the fifteen-parameter field pays $8.8$ more
units of prior penalty to buy $33.5$ more units of misfit reduction.
