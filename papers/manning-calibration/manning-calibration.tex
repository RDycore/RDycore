...
the prior turns the choice into an arithmetic consequence of a quantity
the domain team can argue about.
...
$\sigma_\alpha$ instead treats the classes alike, and turns the prior
into a question the domain expert can answer, the percentage by which
the lookup can be wrong.
...
puts on the lookup, $\sigma_\alpha = 0.30$. Its first quasi-Newton
step drove seven of the fifteen classes to a bound (developed-open,
...
same route, and the route is the problem. \emph{At the width the
lookup deserves, this observable cannot deliver a defensible
