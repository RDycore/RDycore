...
three recovers $84\%$ of what fifteen achieve. What the three reach is
the same at every prior width we ran: developed-medium and woody
wetland at $0.3\times$ their lookup values, which no land-cover table
defends. The marks are asking friction to compensate for an error that
is not in the friction.
...
calibration under the fractional prior at the width the domain team
considers honest, $\sigma_\alpha = 0.30$. Its first quasi-Newton step
drove seven of the fifteen classes to a bound --- developed-open,
developed-low and pasture to $\alpha = 3$, developed-medium,
developed-high, shrub and woody wetland to $\alpha = 0.3$ --- and after
three iterations developed-medium and shrub were still on the floor
($n = 0.036$ and $0.0345$, smoother than concrete) with developed-low at
twice its lookup value, for a peak-WSE MAE of $0.6154$\,m
(Table~\ref{tab:ladder}) against the $0.6116$\,m the absolute prior
reached in nine. A wider prior reaches the same fit sooner, by the
same route, and the route is the problem. \emph{Within a prior wide
enough to be honest, this observable cannot deliver a defensible
fifteen-class roughness field.} What it can deliver is the subject of
...
from every prior we ran --- an answer that puts two of the three
classes on the same $\alpha = 0.3$ floor. We report the prior
width used with every number; its value is a judgment about the
land-cover lookup, not a tuning parameter.
...
\paragraph{The same three classes under the honest prior.}
The three-class run above used the absolute prior. Repeating it under
the fractional prior at $\sigma_\alpha = 0.30$, the width the domain
team calls honest, and at $0.50$ as a bracket above it, gives $0.6290$
...
values are the data's, and they are $n = 0.036$ and $0.029$, below
anything a land-cover table assigns to developed or wetland ground.
Developed-low's value is the prior's: it moves from $1.63\times$ to
$2.34\times$ to $3\times$ its lookup as the width opens while the MAE
moves $0.002$\,m, which is the flat direction the eigenvector
predicted. And the three-parameter field is therefore not physically
defensible either. What the spectrum selects is the calibration the
marks can determine; what they determine is a request for roughness
the lookup cannot supply, at any width we are prepared to call honest
--- evidence that the marks are asking friction to compensate for an
error that lives elsewhere (Section~\ref{sec:icauthority}).
...
of that size in it is plausible. The same group's uncertainty study of this
event finds precipitation and mesh resolution to dominate the simulated
...
high-water marks are the USGS survey served by the Short-Term Network
Flood Event Viewer (\url{https://stn.wim.usgs.gov}); stage records are
USGS NWIS; rainfall is MRMS. \textbf{[Mesh, checkpoint, and rainfall archive: location to be
stated.]}
