...
structures, and GPU backends. One gradient costs about one extra
forward solve regardless of how many parameters are being calibrated,
and every derivative is validated against finite differences of the
...
on how many there are; the observations that carry information in this
flood domain are peak water levels at surveyed high-water marks.
Manning roughness is the dominant tunable parameter of depth-averaged
flood models, and for Hurricane Harvey on a 2.93M-cell Houston-area
mesh, scored against 46 such marks, its ceiling can be measured before
any optimizer runs. Multiplying the whole roughness field by a
...
it cannot be more: one run per land-cover class, perturbing that class
to see how the 46 modelled peaks respond, plus an unperturbed
baseline. The marks carry about \emph{one} parameter's worth of
independent information about roughness, so most of what a
...
turn, since fitting an initial state of millions of unknowns to 46
