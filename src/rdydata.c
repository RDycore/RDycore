...
static PetscErrorCode CheckBoundaryParameters(RDy rdy, const PetscInt boundary_index, PetscInt num_edges) {
  PetscFunctionBegin;
  PetscCall(CheckBoundaryConditionIndex(rdy, boundary_index));
...
// sets Dirichlet boundary values using the data in the strided array values[num_edges * ndof]
...
PetscErrorCode RDySetSedimentDirichletBoundaryValues(RDy rdy, const PetscInt boundary_index, const PetscInt num_edges, const PetscInt num_classes,
                                                     PetscReal* values) {
  PetscFunctionBegin;
...
PetscErrorCode RDySetHeatDirichletBoundaryValues(RDy rdy, const PetscInt boundary_index, const PetscInt num_edges, PetscReal* values) {
  ...
  // the prognostic heat variable is h*T, so we need the boundary heights to
  // convert the given temperatures
  PetscReal *boundary_h, *hT_values;
  PetscCall(PetscCalloc1(num_edges, &boundary_h));
  PetscCall(PetscCalloc1(num_edges, &hT_values));
  PetscCall(GetOperatorBoundaryValues(rdy->operator, boundary, 0, 1, num_edges, boundary_h));
  for (PetscInt e = 0; e < num_edges; ++e) {
    hT_values[e] = boundary_h[e] * values[e];
  }

  PetscCall(SetOperatorBoundaryValues(rdy->operator, boundary, comp_offset, 1, num_edges, hT_values));
  PetscCall(PetscFree(hT_values));
  PetscCall(PetscFree(boundary_h));
