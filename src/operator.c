      ...
      case WELL_BALANCING_NONE:
        ...
        if (op->ceed.use_slope_reconstruction) {
          ...
          PetscCall(CreateCeedFluxOperator(op->config, op->mesh, op->num_boundaries, op->boundaries, op->boundary_conditions, &op->ceed.eta_vertices,
                                           &op->ceed.q_right_refresh_op, &op->ceed.q_right_restricted, &op->ceed.flux));
        }
...
PetscErrorCode AddOperatorSourceVariables(Operator* op) {
  ...
  if (CeedEnabled()) {
    ...
    PetscCallCEED(CeedOperatorCompositeGetSubList(op->ceed.source, &source_suboperators));
    for (CeedInt i = 0; i < num_source_suboperators; ++i) {
      PetscCallCEED(CeedOperatorSetField(source_suboperators[i], "sources", src_restriction, CEED_BASIS_NONE, op->ceed.ceed_src_inst));
    }
...
PetscErrorCode CreateOperator(RDyConfig* config, DM domain_dm, RDyMesh* domain_mesh, PetscInt num_comp, PetscInt num_regions, RDyRegion* regions,
  ...
  // set up source instantaneous snapshot and accumulation vectors
  PetscCall(AddOperatorSourceVariables(*operator));
...
PetscErrorCode DestroyOperator(Operator** op) {
  ...
  if (ceed_enabled) {
    ...
    if ((*op)->ceed.q_right_refresh_op) {
      PetscCallCEED(CeedOperatorDestroy(&((*op)->ceed.q_right_refresh_op)));
    }
...
static PetscErrorCode ApplyCeedOperator(Operator* op, PetscReal dt, Vec u_local, Vec f_global) {
  ...
  if (op->config->physics.flow.well_balancing == WELL_BALANCING_BS2002 && flow_only) {
    ...
    PetscCallCEED(CeedVectorSetValue(op->ceed.eta_vertices, 0.0));  // zero eta_vertices vector before use
    PetscCallCEED(CeedOperatorApply(op->ceed.eta_vertices_operator, op->ceed.u_local, op->ceed.eta_vertices, CEED_REQUEST_IMMEDIATE));
...

  {
    ...
    if (op->ceed.use_slope_reconstruction) {
      ...
      PetscCallCEED(CeedOperatorApply(op->ceed.q_right_refresh_op, op->ceed.u_local, op->ceed.q_right_restricted, CEED_REQUEST_IMMEDIATE));
    }
