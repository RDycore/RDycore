...
static PetscErrorCode SetOperatorDomain(Operator *op, DM dm, DM dm_1dof, RDyMesh *mesh) {
  ...
  if (CeedEnabled()) {
    ...
    if (op->config->physics.flow.well_balance == WELL_BALANCE_BS2002) PetscCallCEED(CeedVectorCreate(ceed, mesh->num_cells, &op->ceed.eta_cell));
  }
...
PetscErrorCode DestroyOperator(Operator **op) {
  ...
  if (ceed_enabled) {
    ...
    if ((*op)->config->physics.flow.well_balance == WELL_BALANCE_BS2002) {
      PetscCallCEED(CeedVectorDestroy(&((*op)->ceed.eta_cell)));
    }
