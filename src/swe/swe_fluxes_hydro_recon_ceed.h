#ifndef SWE_FLUXES_HYDRO_RECON_CEED_H
#define SWE_FLUXES_HYDRO_RECON_CEED_H
...
// geom component indices for interior edges (6 components)
#ifndef HR_INTERIOR_GEOM_INDEX_DEFINED
#define HR_INTERIOR_GEOM_INDEX_DEFINED
typedef enum {
  HR_INTERIOR_SN = 0,
  HR_INTERIOR_CN,
  HR_INTERIOR_NEG_L_OVER_AL,
  HR_INTERIOR_L_OVER_AR,
  HR_INTERIOR_ZC_LEFT,
  HR_INTERIOR_ZC_RIGHT,
  HR_NUM_COMP_INTERIOR_GEOM,
} HRInteriorGeomIndex;
#endif

// SWE interior flux Q-function with hydrostatic reconstruction
//
...
//   in[1]: q_left[num_interior_edges][3]
//   in[2]: q_right[num_interior_edges][3]
//
// Output fields:
...
CEED_QFUNCTION_HELPER int SWEFlux_HydroRecon(void *ctx, CeedInt Q, const CeedScalar *const in[], CeedScalar *const out[], RiemannFluxType flux_type) {
  ...
  for (CeedInt i = 0; i < Q; i++) {
    ...
    SWEState qL_rec = {hL_rec, hL_rec * uL, hL_rec * vL};
    ...
    if (qL.h > tiny_h || qR.h > tiny_h) {
      ...
      // hydrostatic pressure correction applied separately to each cell:
...
CEED_QFUNCTION(SWEFlux_HydroRecon_Roe)(void *ctx, CeedInt Q, const CeedScalar *const in[], CeedScalar *const out[]) {
  return SWEFlux_HydroRecon(ctx, Q, in, out, RIEMANN_FLUX_ROE);
}
