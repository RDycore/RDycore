...
PetscErrorCode CreateSWEFluxOperator(Ceed ceed, RDyMesh *mesh,
    PetscInt num_boundaries, RDyBoundary boundaries[num_boundaries],
    RDyCondition boundary_conditions[num_boundaries],
...
// @param [in]  tiny_h the minimum height threshold for water flow
// @param [out] flux_op A pointer to the flux operator to be created
PetscErrorCode CreateSWESourceOperator(Ceed ceed, RDyMesh *mesh,
    RDyMaterial materials_by_cell[mesh->num_cells], PetscReal tiny_h,
...
PetscErrorCode GetWaterSourceFromSWESourceOperator(CeedOperator source_op,
    CeedOperatorField *water_source_field);

// Given a shallow water equations source operator created by
// CreateSWESourceOperator, fetches the field representing the Riemann flux.
PetscErrorCode GetRiemannFluxFromSWESourceOperator(CeedOperator source_op,
    CeedOperatorField *riemann_flux_field);
