...
// schema for single-component (scalar) fields
static const cyaml_schema_field_t component_fields_schema[] = {
    CYAML_FIELD_STRING("value", CYAML_FLAG_OPTIONAL, RDyComponentCondition, expression, 1),
    CYAML_FIELD_STRING("file", CYAML_FLAG_OPTIONAL, RDyComponentCondition, file, 1),
    CYAML_FIELD_ENUM("format", CYAML_FLAG_OPTIONAL, RDyComponentCondition, format, input_file_formats, CYAML_ARRAY_LEN(input_file_formats)),
    CYAML_FIELD_END
};
...
static const cyaml_schema_field_t sediment_condition_fields_schema[] = {
    ...
    CYAML_SEDIMENT_CONDITION_COMPONENT_FIELDS
    CYAML_FIELD_END
};
...
static const cyaml_schema_field_t salinity_condition_fields_schema[] = {
    ...
    CYAML_FIELD_STRING("value", CYAML_FLAG_OPTIONAL, RDySalinityCondition, expression, 1),
  ...
  for (PetscInt i = 0; i < config->num_sediment_conditions; ++i) {
    ...
    for (PetscInt j = 0; j < config->physics.sediment.num_classes; ++j) {
      PetscCheck(sed_cond->classes[j].expression[0] || sed_cond->classes[j].file[0], comm, PETSC_ERR_USER,
                 "Missing specification for sediment class %" PetscInt_FMT " in sediment_conditions.%s", j, sed_cond->name);
    }
  }
