...
PetscErrorCode RDyDestroyVectors(RDy *rdy) {
  ...
  if ((*rdy)->udot_global) PetscCall(VecDestroy(&((*rdy)->udot_global)));
  if ((*rdy)->udot_local) PetscCall(VecDestroy(&((*rdy)->udot_local)));
  if ((*rdy)->ijacobian) PetscCall(MatDestroy(&((*rdy)->ijacobian)));
